...
class Login {
    ...
    async entrar() {
        ...
        this.user = await LoginModel.findOne({ email: this.body.email }) //Porcura um registro no BD com o email informando.

        if(!this.user){
            this.msgErrors.push("Usuário inválido!");
            return;
        }

        if(!bcryptjs.compareSync(this.body.senha, this.user.senha)){
            this.msgErrors.push("Senha inválida!");
            this.user = null;
            return;
        }
